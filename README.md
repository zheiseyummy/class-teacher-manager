...
- 教师个性化：首次使用可填写教师姓名、学校、任教学科、常用班级和默认学期；首页问候与侧栏个人标记会同步更新。当前正式版本为 `v1.0.0`。
...
```text
dist-python/ClassTeacherManager-v1.0.0-win64.zip
```

同目录会生成 `.zip.sha256` 校验文件。EXE 文件属性、程序侧栏和新建备份清单中的版本号均读取 `config.py` 的统一版本信息。
